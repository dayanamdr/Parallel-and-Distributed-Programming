...
public class Main {
    ...
    private static final int NR_THREADS = 12;
    private static final int NR_TASKS = 6;
    ...
    public static void main(String[] args) throws IOException, InterruptedException, ExecutionException {
        ...
        executorService.submit(Main::diagnosticsTread);
    ...
    public static Pair<Integer, Matrix> searchParallel(Matrix current, int numSteps, int bound, int nrThreads)
        ...
        for (Matrix next : moves) {
            Future<Pair<Integer, Matrix>> f = executorService.submit(
                    () -> searchParallel(next, numSteps + 1, bound, nrThreads / moves.size()));
            futures.add(f);
        }
